...
Botson is a Go-based agent framework built on Google's **ADK v2**. As of 2026-07, it's one **core process** (the Gemini model, agent registry, session/artifact services) exposed over an embedded **NATS server**, and NATS is the *only* way anything talks to it. There is no TUI, no tray, and no CLI subcommand that manages agent/session/settings/script state directly — see "Unified core architecture" below for why, and what the two NATS subject namespaces cover.

---

## Project structure

- **`/cmd`**: application entry points. `botson` is the only one.
  - **[`/botson`](./cmd/botson/)**: the primary application (ships as `botsonv2-<os>-<arch>`) — a small Cobra CLI with exactly two subcommands: `core` (the service itself, plus `start`/`stop`/`status`) and `setup install` (writes the initial `config.json`). Nothing else lives in this binary.
- **`/internal`**: main application packages.
  ...
  - **[`/natsapi`](./internal/natsapi/)**: the server side of Botson's own NATS API — the `botson.*` subjects covering settings, custom-agent CRUD, named scripts, and dashboard-shaped session listing/inspection. See `subjects.go` for the full subject table. The standard ADK surface (list-apps, sessions, running a turn, A2A) is a separate namespace, `adk.*`, fronted by an *imported* [`github.com/Savs-Agents/NATS-ADK-Proxy`](https://github.com/Savs-Agents/NATS-ADK-Proxy) — not implemented in this repo at all; see "Unified core architecture" below.
...
- `github.com/Savs-Agents/NATS-ADK-Proxy` — fronts the standard ADK REST/A2A surface (`adk.*`) over the same NATS connection; a sibling project, not vendored code, so it's kept up to date via normal Go module versioning
...
- Import direction: `cmd/botson` → `internal/natsapi` → `internal/management` → `internal/agent` → `internal/tools` → `internal/config`. `internal/tools` must never import `internal/management` or `internal/agent` (it would cycle back through `internal/agent`'s import of `internal/tools`) — shared logic those layers both need (e.g. `Mask`) belongs in `internal/config` instead, not `internal/management`.
