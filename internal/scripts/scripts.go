...
// internal/config) with no dependency on internal/agent or internal/management, so
// both the CLI (cmd/botson) and the runScript/saveScript agent tools
// (internal/tools) can depend on it directly without an import cycle.
package scripts
...
import (
	...
	"botsonv2/internal/procutil"
)

var nameRegex = regexp.MustCompile(`^[a-zA-Z0-9_-]+$`)

// Sentinel errors so callers (CLI, agent tool) can map failures without
// string-matching error text -- same pattern as internal/management's agent
// errors.
var (
