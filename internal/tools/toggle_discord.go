...
// plain in-process function call (internal/interface/discord's singleton),
// not a new OS process, so it's fast and the agent can do it for the
// user without leaving the conversation. Calls internal/interface/discord
// directly rather than going through internal/management, since
// internal/management already depends on internal/agent, which depends on this
// package (internal/tools) -- routing through it here would create an import
